<template>
    <nav navigation :class="menuContainerCSS" class="cursor-pointer" :ref="el.id" :id="el.id" :style="background(el)">
        <span  @click="menu_responsive=!menu_responsive"><icon-extra v-if="responsive" class="burger-icon md:hidden z-highest fixed top-0 m-1 text-3xl" :class="menu_responsive?el.css.items:el.css.items" :icon="menu_responsive?el.icons.back:el.icons.burger"/></span>
        <!-- desktop -->
